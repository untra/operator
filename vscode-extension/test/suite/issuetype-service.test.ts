...
suite("IssueType Service Test Suite", () => {
  ...
  suite("constructor and defaults", () => {
    ...
    test("uses provided baseUrl", async () => {
      ...
      assert.ok(fetchStub.calledWith(`${customUrl}/api/v1/issuetypes`), "Should use custom URL");
    });
  });
  ...
  suite("extractTypeFromId()", () => {
    ...
    test("handles ticket IDs with extra segments", () => {
      ...
      assert.strictEqual(service.extractTypeFromId("FEAT-123-title-here"), "FEAT");
      assert.strictEqual(service.extractTypeFromId("FIX-456-bug-description"), "FIX");
    });
  });
  ...
  suite("parseTicketFilename()", () => {
    ...
    test("parses the canonical timestamped filename", () => {
      ...
      const result = service.parseTicketFilename("20260807-1200-FEAT-operator-api-tree.md");
      assert.strictEqual(result.type, "FEAT");
  ...
  suite("setBaseUrl()", () => {
    test("updates the base URL", async () => {
      ...
      assert.ok(fetchStub.calledWith("http://newurl:9000/api/v1/issuetypes"), "Should use new URL");
    });
  });
  ...
  suite("refresh()", () => {
    test("fetches issue types from API and updates types", async () => {
      ...
      fetchStub.resolves(new Response(JSON.stringify(apiResponse), { status: 200 }));
    ...
    test("keeps defaults when API is unavailable", async () => {
      ...
      assert.ok(
        (outputChannel.appendLine as sinon.SinonStub).calledWith(sinon.match(/API unavailable/)),
      );
    });
  ...
  suite("glyph to icon mapping", () => {
    test("maps all default glyphs to icons", () => {
      ...
      for (const [type, expectedIcon] of Object.entries(glyphMap)) {
        ...
        assert.strictEqual(icon.id, expectedIcon, `${type} should map to ${expectedIcon}`);
      }
    });
  });
