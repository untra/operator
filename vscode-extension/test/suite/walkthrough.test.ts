...
import type { KanbanEnvResult } from "../../src/walkthrough";
import {
  checkKanbanEnvVars,
  checkLlmToolInPath,
...
suite("Walkthrough Test Suite", () => {
  ...
  suite("checkLlmToolInPath()", () => {
    test("returns false for non-existent tool", async () => {
      const result = await checkLlmToolInPath("definitely-not-a-real-tool-12345");
  ...
  suite("validateWorkingDirectory()", () => {
    test("returns false for non-existent path", async () => {
      const result = await validateWorkingDirectory("/definitely/not/a/real/path/12345");
      assert.strictEqual(result, false);
    });
