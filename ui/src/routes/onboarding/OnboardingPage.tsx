...
export function OnboardingPage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api.refreshCsrf(), api.setupStatus(), api.setupSteps(), api.integrations(), api.setupCollections()])
      .then(([, nextStatus, nextSteps, nextIntegrations, nextCollections]) => {
        if (!active) { return undefined; }
        if (nextStatus.initialized) { void navigate('/', { replace: true }); return undefined; }
      ...
      .catch((cause: unknown) => active && setError(cause instanceof Error ? cause.message : 'Could not load setup'));
    return () => { active = false; };
  }, [api, navigate]);

  const walk = useMemo(() => visibleSteps(steps.map((step) => step.slug), draft), [draft, steps]);
  ...
  function addExport(value: string) {
    setExports((currentExports) => currentExports.includes(value) ? currentExports : [...currentExports, value]);
  }
  ...
  function next() {
    ...
    if (current.slug === 'execution-target' && draft.executionTarget.kind === 'coder' && (!draft.executionTarget.name.trim() || !draft.executionTarget.template.trim())) {
    ...
    try {
      await api.initializeSetup({
        ...
        hosted_collections: draft.hostedCollectionIds.map((id) => {
          ...
          if (!collection) { throw new Error(`Collection ${id} is no longer available`); }
          return { id, checksum: collection.checksum };
        }),
      });
      void navigate('/', { replace: true });
    } catch (cause) {
      setError(cause instanceof ApiError ? cause.message : cause instanceof Error ? cause.message : 'Initialization failed');
    } finally {
      setBusy(false);
    }
  }
  ...
  return <main className={styles.page}>
    <aside className={styles.sidebar}><div className={styles.brand}>Operator</div><ol>{walk.map((slug, index) => { const metadata = steps.find((step) => step.slug === slug); return <li key={slug} className={index === currentIndex ? styles.active : index < currentIndex ? styles.complete : ''}><span>{index + 1}</span>{metadata?.name ?? slug}</li>; })}</ol></aside>
    <section className={styles.content}>
      <header><p>Step {currentIndex + 1} of {walk.length}</p><h1>{current.name}</h1><p>{current.description}</p></header>
      {error && <div className={styles.error}>{error}</div>}
      <div className={styles.body}><Step api={api} status={status} integrations={integrations} collections={collections} draft={draft} setDraft={setDraft} exports={exports} addExport={addExport} /></div>
      <footer><button type="button" disabled={currentIndex === 0 || busy} onClick={() => { setError(null); setCurrentSlug(walk[currentIndex - 1]); }}>Back</button>{currentIndex === walk.length - 1 ? <button type="button" className={styles.primary} disabled={busy} onClick={initialize}>{busy ? 'Initializing…' : 'Initialize workspace'}</button> : <button type="button" className={styles.primary} onClick={next}>Continue</button>}</footer>
    </section>
  </main>;
}
