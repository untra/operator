...
import { OperatorApiClient, discoverApiUrl, type KanbanProjectInfo } from "./api-client";
 ...
 */
export async function onboardJira(context: vscode.ExtensionContext): Promise<void> {
  ...
  void vscode.window.showInformationMessage(`Jira configured! Run Operator to activate.`);
  ...
  const apiKey = await new Promise<string | undefined>((resolve) => {
    ...
    input.onDidTriggerButton((button) => {
      if (button === openLinearSettings) {
        void vscode.env.openExternal(vscode.Uri.parse("https://linear.app/settings/api"));
      }
    });
 ...
 */
export async function onboardLinear(context: vscode.ExtensionContext): Promise<void> {
  ...
  await syncAndNudgeIssueTypes(
    "linear",
    "default",
    `Linear ${selectedTeam.description ?? selectedTeam.label}`,
  );
}
  ...
  const token = await new Promise<string | undefined>((resolve) => {
    ...
    input.onDidChangeValue((value) => {
      if (value && !hasRecognizedGithubTokenPrefix(value)) {
        input.validationMessage = 'GitHub tokens start with "ghp_", "github_pat_", or "gho_"';
      } else {
        input.validationMessage = "";
      }
    });
    ...
    input.onDidTriggerButton((button) => {
      if (button === openGithubSettings) {
        void vscode.env.openExternal(vscode.Uri.parse("https://github.com/settings/tokens"));
      }
    });
 ...
 */
export async function onboardGithub(context: vscode.ExtensionContext): Promise<void> {
 ...
 * the right flow.
 */
export async function startKanbanOnboarding(context: vscode.ExtensionContext): Promise<void> {
  ...
  const choice = await vscode.window.showQuickPick<ProviderPickItem>([...providerItems, skipItem], {
