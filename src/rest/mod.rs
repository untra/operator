...
async fn shutdown_signal(state: state::ApiState) {
    ...
    let config = state.config();
    let drain_deadline =
        Instant::now() + Duration::from_secs(config.rest_api.shutdown_drain_seconds);
    tracing::info!(
    ...
    let cleanup_deadline =
        Instant::now() + Duration::from_secs(config.rest_api.shutdown_cleanup_seconds);
...
fn interrupt_agents_for_shutdown(config: &crate::config::Config) -> Result<()> {
    ...
    let mut app_state = crate::state::State::load(config)?;
    ...
    for agent in &mut app_state.agents {
        ...
        if remote {
            ...
            agent.last_message = Some("Remote work preserved during Operator shutdown".to_string());
            continue;
        }
