...
mod git_cli_readonly_tests {
    ...
    async fn test_repo_root_detection() {
        ...
        assert!(
            ...
            "Root should contain 'operator': {root}"
        );
    ...
    async fn test_get_remote_url() {
        ...
        assert!(
            ...
            "Remote URL should reference operator: {url}"
        );
...
mod git_cli_remote_tests {
    ...
    async fn test_remote_branch_exists_main() {
        ...
        eprintln!("main exists: {main_exists}, master exists: {master_exists}");
    }
...
mod git_cli_branch_lifecycle_tests {
    ...
    async fn test_branch_create_push_delete_lifecycle() {
        ...
        assert!(
            delete_remote.is_ok(),
            "Remote delete should succeed: {delete_remote:?}"
        );
        ...
        assert!(
            delete_local.is_ok(),
            "Local delete should succeed: {delete_local:?}"
        );
...
mod git_cli_worktree_tests {
    ...
    async fn test_worktree_add_remove_lifecycle() {
        ...
        eprintln!("Branch: {branch_name}, Base: {default_branch}");
        ...
        assert!(
            add_result.is_ok(),
            "Worktree add should succeed: {add_result:?}"
        );
        ...
        assert!(
            remove_result.is_ok(),
            "Worktree remove should succeed: {remove_result:?}"
        );
...
mod git_cli_empty_repo_tests {
    ...
    async fn test_create_worktree_empty_repo_fails() {
        ...
        assert!(
            ...
            "Error should mention 'no commits': {err}"
        );
    }
}
