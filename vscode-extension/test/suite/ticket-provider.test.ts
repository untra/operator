...
`;
    await fs.writeFile(path.join(tmpDir, "queue", QUEUE_CARD.filename), ticketMd);
    ...
    sandbox.stub(client, "getKanban").resolves(board({ queue: [QUEUE_CARD], total_count: 1 }));
  ...
  test("a ticket without a frontmatter id falls back to the filename", async () => {
    ...
    const provider = new TicketTreeProvider("queue", new IssueTypeService(mockOutputChannel()));
